...
def search(request):
    if request.method == 'GET':
        ...
        query2 = request.GET.get("email")
        query3 = request.GET.get("area_name")
        query4 = request.GET.get("extension_no")
        print(query)
        if query:
                queryset = queryset.filter(
                    Q(name__icontains=query) |
                    Q(extension_number__contains=query)|
                    Q(email__contains=query)|
                    Q(contact_name__icontains=query)

                ).distinct()
        if query1:
            queryset = queryset.filter(Q(name__icontains=query1)
                                       ).distinct()
        if query2:
            queryset = queryset.filter(Q(email__icontains=query2)
                                           ).distinct()
        if query3:
            queryset = queryset.filter(Q(contact_name__icontains=query3)
                                           ).distinct()
        if query4:
            queryset = queryset.filter(Q(extension_number__contains=query4)
                                           ).distinct()
        paginator = Paginator(queryset, 10)  # Show 25 contacts per page
        page = request.GET.get('page')
        contacts = paginator.get_page(page)

    return render(request,"contact/search.html",{"name":contacts})
...
def emergency_detailsupdate(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            emergency.contact_name = request.POST.get('contact_name')
            emergency.extension_number = request.POST.get('extension_number')
            emergency.name = request.POST.get('name')
            emergency.email = request.POST.get('email')
            emergency.image = request.FILES.get('image')
            emergency.save()
            return redirect('emergency:emergencyDetails_list')
    else:
...
#user
class UserEmergencyDetailsList(TemplateView):
    model = Contact
    context_object_name = 'useremergencyDetails_list'
    template_name = 'contact/useremergency_details_list.html'
    paginate_by = 25
    ...
    def get_queryset(self):
        ...
        query2 = self.request.GET.get("name")
        query3 = self.request.GET.get("email")
        query4 = self.request.GET.get("area_name")
        query5 = self.request.GET.get("extension_no")
        # print(query)
        if query:
            queryset = queryset.filter(
        ...
        if query2:
            queryset = queryset.filter(
                Q(name__contains=query2)
            ).distinct()
        if query3:
            queryset = queryset.filter(
                Q(email__contains=query3)
            ).distinct()
        if query4:
            queryset = queryset.filter(
                Q(contact_name__contains=query4)
            ).distinct()
        if query5:
            queryset = queryset.filter(
                Q(extension_number__contains=query5)
            ).distinct()
        paginator = Paginator(queryset, 2500)  # Show 25 contacts per page
    ...
    def get_context_data(self, **kwargs):
        ...
        branches = Branch.objects.all()
        # print('branch=', branch_id)
        context = super(UserEmergencyDetailsList, self).get_context_data(**kwargs)
        context['EmergencyDetails'] = self.get_queryset()
        context['delete_str'] = "emergency:emergencyDetails_delete"
        context['update_str'] = "emergency:emeregcy_update"
        context['refresh_str'] = "emergency:emeregcy_active"
